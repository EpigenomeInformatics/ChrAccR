...
permutePWMatrix <- function(pwm, nperm = 100) {
  ...
  if (class(pwm) == "PFMatrix") {
    res <- lapply(seq_len(nperm), FUN = function(i) {
      TFBSTools::permuteMatrix(pwm, type = "intra")
    })
...
getMotifDistMat.jaspar <- function(motifIds = NULL, scoreCol = "Ncor") {
  ...
  if (file.exists(fn)) {
    ...
    extractJasparId <- function(ss) {
      sapply(strsplit(ss, "_"), FUN = function(x) {
        x[length(x)]
      })
    }
  ...
  if (motifs == "jaspar") {
    if (clusterMethod == "pam") {
      if (k < 1) {
        ...
        clustAssignL <- lapply(clustRes.pam$medoids, FUN = function(mm) {
          names(clustAssign)[clustAssign == mm]
        })
