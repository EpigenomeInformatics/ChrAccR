...
getGroupsFromTable <- function(tt, cols = NULL, minGrpSize = 2, maxGrpCount = nrow(tt) - 1) {
  ...
  for (j in 1:ncol(tt)) {
    ...
    if (allNA) {
      ...
      rr <- rr[sapply(rr, function(x) {
        !any(is.na(x))
      })] # ignore levels that are missing in the dataset
...
#' @noRd
custom_cicero_cds <- function(cds,
                              reduced_coordinates,
                              k = 50,
  ...
  if (!is.null(summary_stats)) {
    ...
    assertthat::assert_that(
      sum(vapply(summary_stats, function(x) {
        !(is(monocle3::pData(cds)[, x], "numeric") | is(monocle3::pData(cds)[, x], "integer"))
      }, 1)) == 0,
  ...
  cell_sample_map <- lapply(seq_len(nrow(cell_sample)), function(x) rownames(reduced_coordinates)[cell_sample[x, ]]) %>%
    Reduce("rbind", .) %>%
    data.frame()
