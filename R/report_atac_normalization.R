...
setMethod(
  ...
  function(.object,
    ...
    regCountTab <- data.frame(
      ...
      "normalization" = sapply(getRegionTypes(.object), FUN = function(rt) {
    ...
    for (rt in regTypes) {
      ...
      for (sid in sampleIds.ext) {
        pp <- ggplot(df2p[df2p[, "sampleId"] == sid, ]) +
          aes_string(x = paste(countLabel, "unnormalized", sep = "_"), y = paste(countLabel, "normalized", sep = "_")) +
          coord_fixed() +
          geom_abline(intercept = 0, slope = 1, color = "#A0A0A0") +
      ...
      hmu <- ComplexHeatmap::Heatmap(qmU,
        name = paste("Unnormalized", countLabel),
        col = colScheme,
      ...
      hmn <- ComplexHeatmap::Heatmap(qmN,
        name = paste("Normalized", countLabel),
        col = colScheme,
