...
setMethod(
  ...
  function(.object,
    ...
    wasTransformed <- sapply(regionTypes, FUN = function(rt) {
      length(.object@countTransform[[rt]]) > 0
    })
    ...
    diffTabL <- lapply(regionTypes, FUN = function(rt) {
      ...
      ll <- lapply(1:nrow(compTab), FUN = function(i) {
        dat <- getDiffAcc(
          .object, rt, compTab[i, "compCol"],
          grp1Name = compTab[i, "grp1Name"], grp2Name = compTab[i, "grp2Name"],
