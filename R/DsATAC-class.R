...
setMethod(
  ...
  function(.object,
    ...
    if (is.list(mergeGroups)) {
      ...
      mgL <- lapply(rownames(phm), FUN = function(mg) {
        which(mergeGroups == mg)
      })
    ...
    if (countAggrFun == "sum") {
      ...
      if (.object@diskDump) {
        mergeFun <- function(X) {
          DelayedArray::rowSums(X, na.rm = TRUE)
        }
      } else if (.object@sparseCounts) {
        mergeFun <- function(X) {
          Matrix::rowSums(X, na.rm = TRUE)
        }
      }
...
setMethod(
  ...
  function(.object,
    ...
    if (method == "macs2_summit_fw_no") {
      ...
      samplePrefixes <- sapply(samples, FUN = function(x) {
        getHashString(pattern = x)
      })
...
setMethod(
  ...
  function(.object,
    ...
    pp <- ggplot(insSizeDf) +
      aes(x = insertionSize, y = ..count..) +
      geom_density(alpha = .2, fill = "#8c1515") +
      xlim(c(0, 700)) +
      ylab("#fragments")
    return(pp)
  }
...
setMethod(
  ...
  function(.object,
    ...
    pp <- ggplot(tssCountDf) +
      aes_string(x = "pos", y = "countNorm") +
      geom_vline(xintercept = c(0), color = "#4d4f53") +
