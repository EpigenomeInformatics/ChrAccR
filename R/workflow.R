...
makeReportIndex <- function(reportDir, anaName = "ChrAccR analysis", reportIds = character(0)) {
  ...
  stext <- c(
    "<table class=\"pipeline\" style=\"background-image:url(index_images/dna.png);\">", "<colgroup>",
...
run_atac_filtering <- function(dsa, anaDir) {
  ...
  filterStats$regionStats <- data.frame(
    before = sapply(regTypes, FUN = function(rt) {
      getNRegions(dsa, rt)
    })
  )
  ...
  if (reqSamples > 0 && thresh > 0 && !isSingleCell) {
    ...
    filterStats$regionStats[, "after_region_covg"] <- sapply(regTypes, FUN = function(rt) {
      getNRegions(dsf, rt)
    })
    logger.completed()
  }
  ...
  if (length(exclChroms) > 0) {
    ...
    filterStats$regionStats[, "after_chromosomes"] <- sapply(regTypes, FUN = function(rt) {
      getNRegions(dsf, rt)
    })
    logger.completed()
  }
  ...
  filterStats$regionStats[, "after"] <- sapply(regTypes, FUN = function(rt) {
    getNRegions(dsf, rt)
  })
