...
setMethod(
  ...
  function(.object,
    ...
    numericCols <- sapply(colnames(sannot), FUN = function(cn) {
      is.numeric(sannot[, cn])
    })
    ...
    if (isSingleCell) {
      ...
      pcRemL <- list(
        it0 = setdiff(
          1:ncol(dre$iterationData$iteration0$pcaCoord),
          dre$iterationData$iteration0$pcs
        ),
        it1 = setdiff(
          1:ncol(dre$iterationData$iteration1$pcaCoord),
      ...
      if (any(doSubsample)) {
        txt <- c(
          txt, " The following region types have been subsampled to ", nSub, " features: ",
