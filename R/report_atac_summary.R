...
setMethod(
  ...
  function(.object,
    ...
    regCountTab <- data.frame(
      "#regions" = sapply(getRegionTypes(.object), FUN = function(rt) {
        getNRegions(.object, rt)
      }),
      "transformations" = sapply(getRegionTypes(.object), FUN = function(rt) {
        paste(rev(.object@countTransform[[rt]]), collapse = " -> ")
      }),
      check.names = FALSE
    )
    ...
    if (isSingleCell) {
      ...
      sampleStatsTab[, "nFragments"] <- tapply(summaryDf[, "nPass"], summaryDf[, "sample"], FUN = function(x) {
        sum(x, na.rm = TRUE)
      })[sampleStatsTab[, "sample"]]
      for (cn in cns) {
        infix <- ""
      ...
      if (all(c("tssEnrichment") %in% cns)) {
        txt <- c(
          "The plot below shows two frequently used statistics that can be used to asses per-cell quality. ",
        ...
        plotL <- lapply(1:length(sampleIds), FUN = function(i) {
          ...
          pp <- ggplot(df2p) +
            aes_string(x = "log_nPass", y = "tssEnrichment", color = "pointDens")
      ...
      if (hasFragments) {
        ...
        pp <- ggplot(qcTab) +
          aes(nFragments, tssEnrichment) +
          geom_point() +
          geom_text(aes(label = sample), size = 1)
